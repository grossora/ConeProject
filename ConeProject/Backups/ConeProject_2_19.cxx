...
namespace larlite {
  ...
  bool ConeProject::initialize() {
    ...
    //
        ...
        cratio0 = new TH2D("cratio0","Ratio of Contained Charge / Total Charge; Length of Shower Axis; Charge: Contained/Deposit ",100,0,AxisLength,100,0,1);
        cratio1 = new TH2D("cratio1","Ratio of Contained Charge / Total Charge; Length of Shower Axis; Charge: Contained/Deposit",100,0,AxisLength,100,0,1);
        cratio2 = new TH2D("cratio2","Ratio of Contained Charge / Total Charge; Length of Shower Axis; Charge: Contained/Deposit",100,0,AxisLength,100,0,1);

        DangdegDrad0 = new TH2D("das0","bsaha",100,0,AxisLength,100,0,1);
        DangdegDrad1 = new TH2D("das1","bsaha",100,0,AxisLength,100,0,1);
        DangdegDrad2 = new TH2D("das2","bsaha",100,0,AxisLength,100,0,1);
...
//=================================================
	...
	//$$//$$ Define the Cone //
        ...
        double length = AxisLength;// length of the cone axis
...
// - Bring in Hits 
		...
		PxHitsVect[0] = pxhits0;
		PxHitsVect[1] = pxhits1;
		PxHitsVect[2] = pxhits2;
...
//=================================================
	...
	if(coneintpc){
		...
		for(unsigned int a=0; a<nplanes ; a++){
//		std::cout<<"This is the fraction for a Plane "<< a <<" Ratio is "<< Contchargebyplane[a]/chargebyplane[a]<< std::endl;
		double ratio;
...
/*
	...
	if(coneintpc){
		...
		bool Walk = true;
		for( unsigned int a = 0 ; a<nplanes; a++){
		auto WalkA = fgeoconic.walkable(StartConePos, StartConeDir, RandomLength, openingangle, a ,smoothness);
			if(!WalkA){// goto tryagain_fit;
			// walk use continue 
			Walk = false;
				}
			}
			...
			if(Walk){
		// this means things are walkable.... so let's proceed
		for( unsigned int a = 0 ; a<nplanes; a++){
                auto polyproj = fgeoconic.ConicalFeatures(StartConePos, StartConeDir, RandomLength, openingangle, a ,smoothness);
                std::vector<larutil::PxHit> contp = fgeoconic.PolyContain(PxHitsVect[a], polyproj);
                ConePolygonProjection[a] = polyproj;
...
//std::cout<<" Made it to the FIT PART  B "<<std::endl;

		// now I should have the contained Pxhitsvector. 
			AxisSEpt = fconeproj.startendpt(StartConePos,StartConeDir,RandomLength);
			//This gets me the slope and cept for each cone axis in each plane	
			ConeAxisSlopeCept = fconeproj.ConeAxisSC(StartConePos,StartConeDir,RandomLength);
			// This Gets me the Fit of the balances charge
                        recofit0 = fctools.ChargeDistFit(ContainedPxHitsVect[0],ConeAxisSlopeCept[0],-1.0/ConeAxisSlopeCept[0].first);
                        recofit1 = fctools.ChargeDistFit(ContainedPxHitsVect[1],ConeAxisSlopeCept[1],-1.0/ConeAxisSlopeCept[1].first);
                        recofit2 = fctools.ChargeDistFit(ContainedPxHitsVect[2],ConeAxisSlopeCept[2],-1.0/ConeAxisSlopeCept[2].first);
			/////// 
double dangled0;
double dangled1;
double dangled2;
			...
			dangled0 = atan(recofit0.first) - atan(ConeAxisSlopeCept[0].first);
				}
			if(!((recofit0.first>0.0 && ConeAxisSlopeCept[0].first>0.0)||(recofit0.first<0.0 && ConeAxisSlopeCept[0].first<0.0)) ){
			dangled0 = atan(recofit0.first) + atan(ConeAxisSlopeCept[0].first);
				}
			if((recofit1.first>0.0 && ConeAxisSlopeCept[1].first>0.0) || (recofit1.first<0.0 && ConeAxisSlopeCept[1].first<0.0) ){
			dangled1 = atan(recofit1.first) - atan(ConeAxisSlopeCept[1].first);
				}
			if(!((recofit1.first>0.0 && ConeAxisSlopeCept[1].first>0.0)||(recofit1.first<0.0 && ConeAxisSlopeCept[1].first<0.0)) ){
			dangled1 = atan(recofit1.first) + atan(ConeAxisSlopeCept[1].first);
				}
			if((recofit2.first>0.0 && ConeAxisSlopeCept[2].first>0.0) || (recofit1.first<0.0 && ConeAxisSlopeCept[2].first<0.0) ){
			dangled2 = atan(recofit2.first) - atan(ConeAxisSlopeCept[2].first);
				}
			if(!((recofit2.first>0.0 && ConeAxisSlopeCept[2].first>0.0)||(recofit2.first<0.0 && ConeAxisSlopeCept[2].first<0.0)) ){
			dangled2 = atan(recofit2.first) + atan(ConeAxisSlopeCept[2].first);
				}


			// now we can plot the difference as a function of contained radiation length

			double das0 =  fabs(dangled0*180/PI);
			double das1 =  fabs(dangled1*180/PI);
			double das2 =  fabs(dangled2*180/PI);
			DangdegDrad0->Fill(RandomLength,das0);
			DangdegDrad1->Fill(RandomLength,das1);
			DangdegDrad2->Fill(RandomLength,das2);
				}// if Walk
			}//if in subcone
		}
...
// This is all good stuff.... just messy
//
/*
			if(coneintpc){
